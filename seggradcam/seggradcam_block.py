...
class SegGradCAM:
    ...
    def featureMapsGradients(self):
        ...
        # submodel for sequential
        # last_layer_output = self.input_model.get_layer('conv2d_input').output
        # conv_submodel_output = self.input_model.get_layer('sequential').get_layer('conv2d').output
        # conv_submodel = tf.keras.models.Model(inputs=self.input_model.get_layer('sequential').input,
        #                                       outputs=conv_submodel_output)
        # conv_submodel_output = conv_submodel(last_layer_output)

        # submodel for resnet
        conv2d_output = self.input_model.get_layer('sequential').get_layer('conv2d').output
        sub_resnet_output = self.input_model.get_layer('resnet50').get_layer('conv5_block3_out').output
        resnet_submodel = tf.keras.models.Model(inputs=self.input_model.get_layer('resnet50').input,
                                                outputs=sub_resnet_output)
        resnet_submodel_output = resnet_submodel(conv2d_output)

        # submodel for model
        conv5_block3_out_output = self.input_model.get_layer('resnet50').get_layer('conv5_block3_out').output
        model_submodel_output = self.input_model.get_layer('model').get_layer('conv2d_16').output
        new_submodel = tf.keras.models.Model(inputs=self.input_model.get_layer('model').input,
                                             outputs=model_submodel_output)
        # new_submodel_output = new_submodel(conv5_block3_out_output)
        new_submodel_output = new_submodel(resnet_submodel_output)
        intermediate_model = tf.keras.models.Model(inputs=self.input_model.input, 
                                                   outputs=[self.input_model.get_layer('sequential').get_layer(self.prop_to_layer).output,
                                                            new_submodel_output])
        ...
        # intermediate_model = tf.keras.models.Model(inputs=self.input_model.input, 
        #                                            outputs=[self.input_model.get_layer('sequential').get_layer(self.prop_to_layer).output, 
        #                                                     self.input_model.get_layer('model').get_layer(self.prop_from_layer).output])
    ...
    def activationMap(self):
        ...
        # print("self.A: ", self.A)
        # print("self.alph_c: ", self.alpha_c)
        # print("cam: ", cam)
