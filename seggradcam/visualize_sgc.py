...
class SegGradCAMplot(SegGradCAM):
    def __init__(self, seggradcam, trainunet=None, next_dict=None, image_id=None, gt=None,
        ...
        self.all_cam = seggradcam.all_cam
        
        if trainunet!=None:
            self.n_classes = 2 # trainunet.trainparam.n_classes
    ...
    def explainBase_cam(self, title1, title1bias, start_save_name, pixel=False):
        """"""
        ...
        #                          self.timestr + ".png"))
        plt.savefig(os.path.join(self.outfolder,
                                 start_save_name + str(self.cls) + '_to_act' + str(self.prop_to_layer) +'_'+
                                 self.timestr + ".png"))
    ...
    def explainBase(self, title1, title1bias, start_save_name, pixel=False):
        ...
        n_channels = self.all_cam.shape[2]  # Assuming self.all_cam has shape (H, W, C)
        
        fig, axs = plt.subplots(1, n_channels+1, figsize=(10 * scale_fig * n_channels, 5 * scale_fig))
        
        for i in range(n_channels):
            cam = self.all_cam[:, :, i]
            
            im = axs[i].imshow(cam, cmap='jet', alpha=0.6)
            axs[i].set_title(f"{title1} - Channel {i}", fontsize=fonts)
            
            # Use plt.colorbar and specify the ax parameter
            jet = plt.colorbar(im, ax=axs[i], fraction=0.046, pad=0.04, ticks=[0, 0.2, 0.4, 0.6, 0.8, 1])
            jet.set_label(label="Importance", size=fonts)
            jet.ax.set_yticklabels([0, 0.2, 0.4, 0.6, 0.8, 1], size=fonts)
            
            if pixel:
                axs[i].scatter(self.roi.j, self.roi.i, color='white', s=scatter_size)
        
        im = axs[-1].imshow(self.cam, cmap='jet',  # vmin=0,vmax=1,
                   alpha=0.6)
        axs[-1].set_title(f"{title1} - All", fontsize=fonts)
        jet = plt.colorbar(im, ax=axs[-1], fraction=0.046, pad=0.04, ticks=[0, 0.2, 0.4, 0.6, 0.8, 1])
        jet.set_label(label="Importance", size=fonts)
        jet.ax.set_yticklabels([0, 0.2, 0.4, 0.6, 0.8, 1], size=fonts)
    ...
    def baseGtPrediction(self, title2, start_save_name_tetra, pixel=False):
        ...
        #                          self.timestr + ".png"))
        plt.savefig(os.path.join(self.outfolder,
                                 start_save_name_tetra + str(self.cls) + '_to_act' + str(self.prop_to_layer) +'_'+
                                 self.timestr + ".png"))
